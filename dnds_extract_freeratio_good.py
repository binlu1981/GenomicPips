...
if sys.version_info[0] > 2:
    standard_id = input("Is your taxon ID is standard as ctx0001 format? Enter YES or No ")
    if standard_id in ["YES","yes","y","Y"]:
        n = int(input("please input the number of species: "))
    else:
        taxon_ids = input("Input the taxon ID seprarated by space ")
        taxon_ids_list = taxon_ids.split()
else:
    standard_id = raw_input("Is your taxon ID is standard as ctx0001 format? Enter YES or No ")
    if standard_id in ["YES","yes","y","Y"]:
        n =int( raw_input("please input the number of species: "))
    else:
        taxon_ids = raw_input("Input the taxon ID seprarated by space ")
        taxon_ids_list = taxon_ids.split()
...
if standard_id in ["YES","yes","y","Y"]:
    ...
    for x in taxon_ids_list:
        dnds_dicts_all_terminals[x] = {}
        dn_dicts_all_terminals[x] = {}
        ds_dicts_all_terminals[x] = {}

for op in glob.glob('*codeml'):
#    gene = op.replace(".fa.merge.ctx_codonalign.branch.codeml","")
    if re.match(r'.*(ENS\w{12}).*',op) is not None:
        gene = re.match(r'.*(ENS\w{12}).*',op).group(1)
    else:
        gene = op
    with open(op) as f:
    ...
    #            species_dnds = re.search(r"w\sratios\sas\slabels\sfor\sTreeView:.+"+species+"\s#(\d+\.\d+)",txt,flags = re.DOTALL|re.S).group(1)
            ...
            for x in taxon_ids_list:
                dnds = re.search(r"^w\sratios\sas\slabels\sfor\sTreeView:\n{1}.+" + x + "\s#(\d+\.\d+)",txt,flags=re.M).group(1)
            # species_dnds = re.search(r"w\sratios\sas\slabels\sfor\sTreeView:.+"+species+"\s#(\d+\.\d+)",txt,flags = re.DOTALL|re.S).group(1)
                dnds_dicts_all_terminals[x][gene] = dnds

                dn = re.search(r'^dN\stree:\n{1}.+' + x + ':\s+(\d+\.\d+)', txt, flags=re.M).group(1)
                dn_dicts_all_terminals[x][gene] = dn

                ds = re.search(r'^dS\stree:\n{1}.+' + x + ':\s+(\d+\.\d+)', txt, flags=re.M).group(1)
                ds_dicts_all_terminals[x][gene] = ds
