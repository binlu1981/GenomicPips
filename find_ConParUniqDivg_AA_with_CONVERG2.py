...
for file in glob.glob('./*'+ext):
    ...
    with open(file) as f:
        ...
        if len(BG_list_user) != 0:
            BG_list = sorted(BG_list_user)
        else:
            BG_list = sorted(list(x for x in set(all_taxon_list).difference(set(FG_list)) if 'node' not in x))
...
#                if seq3[spb][i] != '-':
            ...
            if set(FG_sites).issubset(AA_list) and set(BG_sites) < set(AA_list) and len(set(FG_sites)) == 1 and len(set(FG_sites) & set(BG_sites)) == 0 and len(set(ANC_sites)) == len(set(ANC_list)) and len(set(FG_sites) & set(ANC_sites)) == 0:
#            if set(FG_sites).issubset(AA_list) and set(BG_sites) < set(AA_list) and len(set(FG_sites)) == 1 and len(set(FG_sites) & set(BG_sites)) == 0 and len(set(ANC_sites)) == 2 and len(set(FG_sites) & set(ANC_sites)) == 0:
            ...
            if set(FG_sites).issubset(AA_list) and set(BG_sites) < set(AA_list) and len(set(FG_sites)) == len(set(FG_list)) and len(set(FG_sites) & set(BG_sites)) == 0 and len(set(ANC_sites)) == 1 and len(set(FG_sites) & set(ANC_sites)) == 0:
                count_DIVG += 1
                divergent_dict = {}
                divergent_dict['Gene_ID'] = geneid
                divergent_dict['FG_Taxons'] = FG_list
                divergent_dict['BG_Taxons'] = BG_list
                divergent_dict['ANC_Taxons'] = ANC_list
                divergent_dict['FG_AA'] = FG_sites
                divergent_dict['Position'] = i + 1
                divergent_dict['BG_AA'] = BG_sites
                divergent_dict['ANC_AA'] = ANC_sites
                if len(divergent_dict) != 0:
                    divergent_list.append(divergent_dict)
        if prob_test in ['YES', 'yes', 'Y', 'y']:
        ...
        for du in unique_share_list:
            du["Obs_UNIQ"] = count_UNIQ
        for dd in divergent_list:
            dd["Obs_DIVG"] = count_DIVG
        if prob_test not in ['YES', 'yes', 'Y', 'y']:
            for dcc in convergent_list:
                dcc["Obs_CONV"] = count_CONV
            for dpc in parallel_list:
                dpc["Obs_PARA"] = count_PARA
        unique_share_all.extend(unique_share_list)
        convergent_all.extend(convergent_list)
        parallel_all.extend(parallel_list)
        divergent_all.extend(divergent_list)
...
if len(divergent_all) != 0:
    divergent_df = DataFrame(divergent_all)
#    if not identity_df.empty:
    divergent_df.set_index('Gene_ID').sort_index().to_csv('_'.join(FG_list)+"_divergent_info.csv",header=True,index=True)
else:
    print("No uniquely shared sites were found at interesting taxons, you may want to try other taxons")
